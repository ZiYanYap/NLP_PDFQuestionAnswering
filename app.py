...
# Cache the loaded embedding models
@st.cache_resource
def load_embedding_models():
    return {model_key: HuggingFaceEmbeddings(model_name=model_name, model_kwargs={"trust_remote_code": True})
...
def main():
    ...
    with st.sidebar:
        ...
        selected_embedding_model_name = st.radio("Select an Embedding Model:", list(EMBEDDING_MODEL_NAMES.keys()), on_change=reset_embedding)
        selected_QA_model_name = st.radio("Select a QA Model:", list(QA_MODEL_NAMES.values()), on_change=reset_qa)
            ...
            if uploaded_file is not None:
                if "db" not in st.session_state:
                    ...
                    with st.spinner('Loading embedding model...'):
                        ...
                        embedding_model = load_embedding_models().get(selected_embedding_model_name)
    ...
    if 'db' in st.session_state and 'qa_model' in st.session_state:
        ...
        if prompt := st.chat_input("Ask a question about the resume..."):
            # Track response time from submitting query until response is diplayed
            tic = time.time()
            ...
            toc = time.time()
            print(f"Response generated in {toc - tic:0.4f} seconds")
